...
def loadreadyqueue():
    ...
    while(waiting_queue and waiting_queue[0].p_dept_waiting==total_time):
        if(waiting_queue[0].rem_quantum<quanta):
             readylist.append(waiting_queue[0])
             obj1=process();
             obj1=readylist[len(readylist)-1]
             tempint=len(readylist)-2
             while(tempint!=-1):
                 readylist[tempint+1]=readylist[tempint]
                 tempint-=1
             readylist[0]=obj1
        else:
            readylist.append(waiting_queue[0])
...
def inc_time():
    ...
    if(cpu[0].p_run_time==cpu[0].p_bus_time):
        ...
        if(readylist):
            ...
            del readylist[0]
        elif waiting_queue or obj or waiting_queue:
            while(True):
                total_time+=1
                loadreadyqueue()
                if(readylist):
